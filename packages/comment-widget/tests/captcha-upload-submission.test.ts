...
import { getCaptchaCodeHeader } from '../src/utils/captcha.ts';
import { UploadSession } from '../src/utils/upload-session.ts';

for (const withImages of [false, true]) {
  test(`sends Turnstile token through upload submission (images: ${withImages})`, async () => {
    const requests: {
    ...
    const server = createServer((request, response) => {
      ...
      requests.push({
        token: request.headers['x-turnstile-token'] as string | undefined,
    ...
    try {
      ...
      await new UploadSession().submit(
        ...
        { content: 'test' },
        ids,
        getCaptchaCodeHeader('', 'verified-token'),
        base
      );
